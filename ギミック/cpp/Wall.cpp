			...
			*/
			...
			if (wall_huti[x][y])
			{
					...
					wall_huti[x][y - adjust])
				{
					...
					drawsize.x = conersize;//右縁の大きさx
					drawsize.y = 1;

					drawpos.x = x - (drawsize.x / 2) + 1;//右縁の位置x
					drawpos.y = y + 1;
				}
				...
				{
					drawtype = 2;
					drawsize.x = conersize;//左縁の大きさx
					drawsize.y = 1;

					drawpos.x = x + (drawsize.x / 2);//左縁の位置x
					drawpos.y = y + 1;
				}
					...
					wall_huti[x - adjust][y])
				{
					drawtype = 3;
					drawsize.x = 1.0f;
					drawsize.y = conersize;//下縁の大きさy

					drawpos.x = x + 1;
					drawpos.y = y - (drawsize.y / 2) + 1;//下縁の位置y
				}
				//上壁
				if (!wall_huti[x][y - 1] &&
					wall_huti[x + adjust][y] &&
					wall_huti[x - adjust][y])
				{
					drawtype = 4;
					drawsize.x = 1.0f;
					drawsize.y = conersize;//上縁の大きさy

					drawpos.x = x + 1;
					drawpos.y = y + (drawsize.y / 2);//上縁の位置y
				}
				...
				if (!wall_huti[x + 1][y] &&
					...
					!wall_huti[x + 1][y + 1])
				{
					drawsize.x = conersize;//外角（頂点）の大きさ
					drawsize.y = conersize;//外角（頂点）の大きさ
					drawtype = 9;
				...
				{
					drawtype = 5;
					drawsize = D3DXVECTOR2(conersize, conersize);//内角（頂点）の大きさ
					drawpos.x = x - (drawsize.x / 2)+1;
					drawpos.y = y - (drawsize.y / 2)+1;
				}
				//右上壁
